    ...
    async function compose(message,channel){
        ...
        var ane = message.guild.me.user,
            ...
            cnt = [
              ...
              {name:"Ping : ",value:`\`\`\`${new Date() - message.createdTimestamp}ms\`\`\``,inline:true},
              {name:"__command                                                                                                 .__",value:"```css\n"+cmn+"```"},
              {name:"__.                                                                                                                   .__", value:"we do limit the command channels usage!!",inline:false},
              {name: "limit channels :",value:"```css\n"+limit+"```",inline:false}
            ];
        // console.log(ane);
        ...
        var exampleEmbed = await {
                ...
                description: `I'm just a **radio-stream Selfbot**,\n programmed and owned by \`${client.users.get(owner.id).tag}\``,
                fields:cnt,
                timestamp: new Date()
              };
      return exampleEmbed;
