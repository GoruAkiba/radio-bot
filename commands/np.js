  ...
  execute(message, args, client) {
    ...
    if(!fs.existsSync("./queue.json"))return message.channel.send("queue kosong!!!");
    ...
    fs.readFile("./queue.json",'utf8',(err,e)=>{
      ...
      request.post({
        ...
        url:     post_serv,
        body:   `streamUrl=${encodeURIComponent(queue.uri)}&streamId=${queue.stream_id}`
      }, async (error, response, body)=>{
        console.log(body);
        var {data} = JSON.parse(body);
        var embed = await compose(message,data,queue.name);
    ...
    async function compose(message,data,station){
      ...
      // console.log(name);
        ...
        var exampleEmbed = await {
          ...
          thumbnail:{
            url:"https://cdn.glitch.com/5f7d51b1-406e-43aa-9be8-293ff08f0543%2Fnot%20music.png?v=1579912628468"
          },
          description: `**NowPlaying :** \n${data.song!==""?data.song:"unknow"} *[${data.artist!==""?data.artist:"unknow"}]* \nStation : ${station}`,
          image:{
            url: "https://cdn.glitch.com/5f7d51b1-406e-43aa-9be8-293ff08f0543%2Fgiff.gif?v=1579915986916"
          }
        };
      return exampleEmbed;
    }
